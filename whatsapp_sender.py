...
try:
    ...
    logger.info("✅ pywhatkit available")
except ImportError:
...
class WhatsAppSender:
    def __init__(self):
        ...
        self.recipients = WHATSAPP_RECIPIENTS if WHATSAPP_RECIPIENTS else [WHATSAPP_PHONE]
        
        if not PYWHATKIT_AVAILABLE:
            logger.info("💡 To enable WhatsApp: pip install pywhatkit")
        if self.enabled:
            logger.info(f"📱 WhatsApp enabled for {self.recipients}")
        else:
    ...
    def send_signal(self, signal):
        ...
        if not self.enabled:
            logger.warning("WhatsApp is disabled. Check config.py")
            logger.warning("   WHATSAPP_ENABLED = True")
            logger.warning("   WHATSAPP_PHONE = '+27645471297'")
            return False
    ...
    def _send_message(self, phone, message):
        ...
        try:
            ...
            logger.info(f"📤 Sending WhatsApp message to {phone} at {hour}:{minute}")
...
def test_whatsapp():
    ...
    try:
        ...
        print("✅ Signal sent! Check your WhatsApp.")
        return True
    except Exception as e:
        print(f"❌ Error: {e}")
